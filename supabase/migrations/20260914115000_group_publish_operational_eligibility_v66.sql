-- Groups v66: use the same operational eligibility projection for planning and publishing.
--
-- The Groups UI already builds from get_participant_eligibility, which includes the
-- session-approved operational cohort (including verified awaiting-approval youth and
-- approved local exceptions) while preserving the imported registration history.
-- publish_grouping_plan was still enforcing the older approved + 14-18 DOB rule, so a
-- valid 1,784-youth reviewed structure was being compared against 1,747 legacy-eligible
-- rows and blocked. Keep source registration values untouched and make publish use the
-- canonical operational decision instead.
...
begin
  ...
  -- This is deliberately the same authority used by get_participant_eligibility.
  -- It keeps excluded/non-current youth out while allowing the approved operational
  -- cohort without rewriting imported approval history.
  ...
  values(
    ...
    jsonb_build_object(
      ...
      'eligibility_basis','operational_participant_is_eligible_v66',
