...
begin
  ...
  begin
    ...
    if coalesce((result->>'participant_count')::integer, 0) <> before_grouped then
      raise exception 'v69 dry-run participant count changed from % to %', before_grouped, result->>'participant_count';
    end if;
    if coalesce((result->>'previous_identity_count')::integer, 0) <> before_badges then
      raise exception 'v69 dry-run identity count changed from % to %', before_badges, result->>'previous_identity_count';
    end if;
    if coalesce((result->>'arrived_checkins_preserved')::integer, -1) <> before_checkins then
      raise exception 'v69 dry-run did not preserve arrived check-ins';
    end if;

    raise exception '__v69_intentional_rollback__';
  exception when others then
    if sqlerrm <> '__v69_intentional_rollback__' then
      raise;
    end if;
  end;
  ...
  if (select count(*) from public.companies where session_id = sid) <> before_companies
    ...
    raise exception 'v69 dry-run rollback did not restore the original live structure';
  end if;
end;
$test$;
