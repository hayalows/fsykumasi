...
begin
  ...
  if min_size is null then
    min_size := 8; max_size := 10; groups_target := 2; use_bands := true; avoid_units := true;
  end if;
  ...
  -- During planning, leaders can rehearse head count/check-in and still revise structure.
  -- Once the session leaves planning, real operational activity locks full rebuilds.
  ...
  into supplied_total, distinct_total from supplied;
  ...
  if exists(
    ...
    raise exception 'Grouping plan contains an ineligible participant';
  end if;
  ...
  if use_bands and exists(
    ...
    having count(distinct case
      when p.age between 14 and 15 then '14-15'
  ...
  for company_item in select value from jsonb_array_elements(p_plan) loop
    ...
    for group_item in select value from jsonb_array_elements(company_item -> 'groups') loop
      ...
      where p.session_id = p_session_id
        and p.id in (select value::uuid from jsonb_array_elements_text(group_item -> 'participant_ids'));
    end loop;
  end loop;
  ...
  values(
    ...
    jsonb_build_object(
      'company_count',company_count,'group_count',group_count,'participant_count',participant_total,
...
create index if not exists session_structure_settings_updated_by_idx
  on public.session_structure_settings(updated_by);
create index if not exists staff_company_assignments_assigned_by_idx
  on public.staff_company_assignments(assigned_by);
create index if not exists staff_company_assignments_company_id_idx
