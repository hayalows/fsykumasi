-- Groups v24: publish the same operational cohort the UI builds.
-- The final-roster v21 publish guard used the configured age column range directly,
-- while the rest of operations (and the Groups UI through get_participant_eligibility)
-- uses the canonical DOB/session eligibility rule. On the final Kumasi roster that
-- produced a 1743-person reviewed draft while publish incorrectly expected 1745.
...
begin
  ...
  -- Use the same DOB/session eligibility rule as get_participant_eligibility and
  -- the rest of live operations. Do not substitute the coarse configured age
  -- column range here; that was the source of the reviewed-draft mismatch.
  ...
  where p.session_id=p_session_id
    ...
    and sess.starts_on is not null
    and sess.ends_on is not null
    and extract(year from sess.starts_on)::int-extract(year from d.date_of_birth)::int>=14
    and sess.ends_on<(d.date_of_birth+interval '19 years')::date;
  ...
  if participant_total=0 or supplied_total<>participant_total or distinct_total<>participant_total then
    raise exception 'Reviewed structure has % youth (% unique), but % are currently eligible. Rebuild the structure and publish again.', supplied_total, distinct_total, participant_total;
  end if;
  ...
  if exists(
    ...
    select 1
    from supplied x
    left join public.participants p on p.id=x.participant_id and p.session_id=p_session_id
    left join public.sessions sess on sess.id=p.session_id
    ...
    where p.id is null
       ...
       or p.attendance_status='confirmed_not_attending'
       or d.date_of_birth is null
       or sess.starts_on is null
       or sess.ends_on is null
       or extract(year from sess.starts_on)::int-extract(year from d.date_of_birth)::int<14
       or not (sess.ends_on<(d.date_of_birth+interval '19 years')::date)
  ) then
