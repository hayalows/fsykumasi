...
import { DEFAULT_STRUCTURE_SETTINGS, applyStaffAssignmentPlan, assignCounselorToGroup, loadOperationalStructure, loadStaff, loadStructureSettings, saveStructureSettings, unassignCounselorFromGroup } from "../lib/operations.js";
...
function LiveGroup({group,members,staff,focusGroupId}){
 ...
 return <div className={`groups-v2-group ${group.id===focusGroupId?"context-focus":""}`} id={`group-${group.id}`}>
  <button type="button" onClick={()=>setOpen(value=>!value)} aria-expanded={open}>
   ...
   <span><b>{covered?counselor.name:counselor?"Replacement needed":"Counselor needed"}</b><small>{open?"Hide people":issue||"Counselor"}</small></span>
  </button>
...
function LiveCompany({company,participantsByGroup,staff,focusGroupId,focusCompanyId}){
 ...
 const leadLabel=activeAssistants.length?activeAssistants.map(person=>person.name).join(", "):assistants.length?"Replacement needed":"Assistant Coordinator needed";
 ...
 return <article className={`groups-v2-company ${focused?"context-focus":""}`} id={`company-${company.id}`}>
  <button type="button" className="groups-v2-company-head" onClick={()=>setOpen(value=>!value)} aria-expanded={open}>
   ...
   <span className="groups-v37-company-lead"><small>Assistant Coordinator</small><b>{leadLabel}</b><small>{open?"Hide counselor groups":"Open counselor groups"}</small></span>
  </button>
...
export function Groups({companyIds=null,participants,assignment,onPublish,live=false,canManage=false,sessionId,onNavigatePeople,onSettingsChange,sessionName}){
 ...
 const filtered=useMemo(()=>{
  ...
  return companies.map(company=>{
   const groupRanks=company.groups.map(group=>personSearchRank({name:groupName(group)},text,[companyName(company)]));
   ...
   const companyRank=personSearchRank({name:companyName(company)},text,[company.scriptureReference||"",...company.groups.map(groupName)]);
 ...
 return <section className="page groups-v2 groups-v37">
  <PageHead title={mode==="live"?liveTitle:"Plan groups & companies"} sessionName={sessionName} description={mode==="live"?(scoped?"See your company lead, counselor groups and youth in one place.":"Find a company, counselor group, participant or staff leader."):"Set the group rules, review the proposed structure, then finish staff coverage."} action={mode==="live"&&canManage&&!scoped?<button type="button" className="secondary" onClick={()=>setMode("planning")}>Edit structure</button>:mode==="planning"&&published?<button type="button" className="secondary" onClick={()=>setMode("live")}>Back to live structure</button>:null}/>
