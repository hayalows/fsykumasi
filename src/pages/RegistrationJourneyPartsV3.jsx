...
export function OnSiteDetails({ initialSearch = "", sessionStart, unitDirectory = [], busy, error, onCreate, onCancel, onClose }) {
  ...
  return <form className="regjourney-onsite-form regjourney-onsite-form-v3" onSubmit={submit}>
    <header className="regjourney-sheet-intro regjourney-sheet-intro-v3"><div><span className="kicker">On-site registration · Step 1 of 4</span><h2>Add participant</h2><p>Start with the essentials. Verification, placement and check-in come next.</p></div><LayerCloseButton onClose={onClose} /></header>
    ...
    <section className="regjourney-form-section"><div className="regjourney-form-section-head"><h3>Church unit</h3><span>Stake / district can fill itself</span></div><div className="regjourney-form-grid two">
      ...
      <label className="regjourney-span-2">Stake / district <span>Auto-filled when recognized</span><input value={form.stake} onChange={(e) => set("stake", e.target.value)} placeholder="Will fill from the ward / branch when available" /></label>
    </div></section>
    ...
    <section className="regjourney-form-section"><div className="regjourney-form-section-head"><h3>Contact</h3><span>One phone number is enough to continue</span></div><div className="regjourney-form-grid two">
      ...
      <label>Parent / guardian phone<input type="tel" inputMode="tel" autoComplete="tel" value={form.guardianPhone} onChange={(e) => set("guardianPhone", e.target.value)} /></label>
...
export function PersonJourney({ row, eligibility, identityReadiness, vacancies, groups, companies, housingAssignment, canManageRegistration, busy, error, onVerify, onAssignGroup, onUseVacancy, onCheckin, onArrivalStatus, onDone, onClose }) {
  ...
  return <div className="regjourney-person-flow regjourney-person-flow-v3">
    ...
    {!onsitePending && needsPlacement && canManageRegistration ? (finalized && row.sourceKind === "on_site" ? <VacancyPicker vacancies={vacancies} row={row} busy={busy} onChoose={onUseVacancy} /> : <div className="regjourney-resolution-section"><div className="regjourney-section-head"><div><span className="kicker">{row.sourceKind === "on_site" ? "On-site registration · Step 3 of 4" : "Placement"}</span><h3>Choose a counselor group</h3><p>Best-fit groups appear first. The company follows the group automatically.</p></div></div><GroupPicker groups={groups} companies={companies} row={row} busy={busy} onChoose={onAssignGroup} /></div>) : null}
    ...
    {!onsitePending && !needsPlacement && !ready && row.checkinStatus !== "arrived" && problem ? <div className="regjourney-blocked"><WarningCircle/><div><b>{problem}</b><p>Resolve this eligibility issue before placement or check-in.</p></div></div> : null}
