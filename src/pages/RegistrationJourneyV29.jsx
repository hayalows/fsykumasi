...
export function RegistrationJourneyV29({ view="desk", participants=[], initialGroups=[], initialFilter="", sessionId, capabilities=[], onOperationalDataChanged, onCheckin, onUndoCheckin, onSetOperationalStatus }){
  ...
  const runMutation=async(personId,action,success,{refresh=true,parent=true,onRefreshError}={})=>{
    ...
    try{
      ...
      if(refresh)try{await reload();}catch(err){setError(`Saved, but the roster could not refresh. ${err.message||"Use Refresh to load the latest state."}`);onRefreshError?.(err);}
