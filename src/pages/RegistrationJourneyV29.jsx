...
export function RegistrationJourneyV29({ view="desk", participants=[], initialGroups=[], initialFilter="", sessionId, capabilities=[], onOperationalDataChanged, onCheckin, onUndoCheckin, onSetOperationalStatus }){
  ...
  const checkIn=async(row,keepOpen=false)=>{if(!canCheckin)return;try{const housing=housingByPerson.get(row.participantId);const success=housing?`${row.fullName} is checked in · Housing: ${housing.roomName}.`:`${row.fullName} is checked in. Housing can assign a room now.`;const state=await runMutation(row.participantId,()=>onCheckin?onCheckin(row.participantId,"arrived"):!sessionId?Promise.resolve({recordedAt:new Date().toISOString()}):recordCheckin({sessionId,participantId:row.participantId,status:"arrived"}),success,{refresh:false,parent:false});setRows(current=>current.map(item=>item.participantId===row.participantId?{...item,checkinStatus:"arrived",checkinRecordedAt:state?.recordedAt||item.checkinRecordedAt||null}:item));if(!keepOpen)focusNext();}catch{}};
  ...
  return <section className={`regjourney regjourney-${view} regjourney-v2 regjourney-v3 regjourney-v4 regjourney-v6 regjourney-v29 regjourney-v30 regjourney-v31`}>
