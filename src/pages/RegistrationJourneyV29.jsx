...
export function RegistrationJourneyV29({ view="desk", participants=[], initialGroups=[], initialFilter="", sessionId, capabilities=[], onOperationalDataChanged }){
  ...
  return <section className={`regjourney regjourney-${view} regjourney-v2 regjourney-v3 regjourney-v4 regjourney-v6 regjourney-v29 regjourney-v30`}>
    {view==="roster"?<><div className="regjourney-roster-head regjourney-roster-head-v2 regjourney-roster-head-v3"><div><span className="kicker">Solutions</span><h2>Resolve the blocker, then return to normal check-in</h2><p>Each participant shows one current blocker and the next useful action. Supporting setup lives in Readiness, not in a second exception queue.</p></div><div><b>{counts.needs_help.toLocaleString()}</b><span>blocked</span></div></div><p className="regjourney-solution-principle-v30"><b>One participant exception queue.</b> Start with All blockers unless you are working a specific task type.</p></>:null}
    ...
    <article className="panel regjourney-worklist regjourney-worklist-v2 regjourney-worklist-v3" aria-busy={initialLoading||refreshing}>
      ...
      <div className="regjourney-list regjourney-list-v2">
        ...
        {!initialLoading&&!loadError&&!visible.length&&!searching?<Empty icon={CheckCircle} title={filter==="ready"?"No one is ready right now":filter==="needs_help"?"Solutions queue is clear":"Nothing in this view"} text={filter==="ready"?"Open Solutions for unresolved records or search for the participant directly.":filter==="needs_help"?"No current participant is blocked by registration, eligibility, placement, identity or arrival follow-up.":"Choose another status or search for a participant."}/>:null}
      </div>
