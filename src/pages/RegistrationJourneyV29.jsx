...
import { DeskFilters, OnSiteDetails, PersonJourney, arrivalLabel, arrivalTone, displaySource, initials, isReady, rowProblem } from "./RegistrationJourneyPartsV4.jsx";
...
function matchesWorkFilter(row,eligibility,filter){
  ...
  if(filter==="ready")return Boolean(eligibility)&&isReady(row,eligibility);
  if(filter==="needs_help")return Boolean(eligibility)&&Boolean(rowProblem(row,eligibility));
...
export function RegistrationJourneyV29({ view="desk", participants=[], initialGroups=[], initialFilter="", sessionId, capabilities=[], onOperationalDataChanged }){
  ...
  const filtered=useMemo(()=>{
    ...
    return currentRows.filter(row=>{
      ...
      if(!text&&["all","expected"].includes(filter)){
        ...
        const aReady=Boolean(ae)&&isReady(a,ae),bReady=Boolean(be)&&isReady(b,be);if(aReady!==bReady)return aReady?-1:1;
        const aProblem=Boolean(ae&&rowProblem(a,ae)),bProblem=Boolean(be&&rowProblem(b,be));if(aProblem!==bProblem)return aProblem?1:-1;
      }
  ...
  const checkIn=async(row,keepOpen=false)=>{if(!canCheckin)return;try{const housing=housingByPerson.get(row.participantId);const success=housing?`${row.fullName} is checked in · Housing: ${housing.roomName}.`:`${row.fullName} is checked in. Housing can now see them in Arrivals waiting.`;await runMutation(row.participantId,()=>recordCheckin({sessionId,participantId:row.participantId,status:"arrived"}),success,{refresh:false,parent:false});setRows(current=>current.map(item=>item.participantId===row.participantId?{...item,checkinStatus:"arrived"}:item));if(!keepOpen)focusNext();}catch{}};
  ...
  return <section className={`regjourney regjourney-${view} regjourney-v2 regjourney-v3 regjourney-v4 regjourney-v6 regjourney-v29`}>
