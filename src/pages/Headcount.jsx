        ...
        <>
          <article className={roundExpanded ? "panel headcount-round-card open" : "panel headcount-round-card"}>
            ...
            {roundExpanded ? <div className="headcount-round-detail">
              ...
              <div className="progressive-toolbar headcount-toolbar"><SearchField value={query} onChange={(value) => { setQuery(value); setVisibleLimit(20); }} label="Search head-count companies" placeholder="Search company"/><div className="filter-chips headcount-filters" role="group" aria-label="Filter head-count companies"><button type="button" className={filter === "awaiting" ? "active" : ""} onClick={() => chooseFilter("awaiting")}>Awaiting {awaiting}</button><button type="button" className={filter === "exception" ? "active" : ""} onClick={() => chooseFilter("exception")}>Exceptions {exceptions}</button><button type="button" className={filter === "reported" ? "active" : ""} onClick={() => chooseFilter("reported")}>Reported {headcount.submissions.length - exceptions}</button><button type="button" className={filter === "all" ? "active" : ""} onClick={() => chooseFilter("all")}>All</button></div></div>
              ...
              <div className="headcount-progressive-list">
                {shownCompanies.map((company) => {
                  ...
                  return <div className={open ? "headcount-company-card open" : "headcount-company-card"} key={company.id}>
                    ...
                    {open ? <div className="headcount-company-detail">{saved ? <div className="saved-headcount-context"><span><small>Last report</small><b>{saved.accounted_count} of {saved.expected_count}</b></span><span><small>Status</small><b>{saved.status === "exception" ? "Exception" : "All accounted for"}</b></span>{saved.note ? <p>{saved.note}</p> : null}</div> : <p className="form-hint">No report has been submitted for this company yet.</p>}{!saved && !headcount.round.closes_at && onSubmit ? <button className="primary headcount-all-here" disabled={busy === company.id} onClick={() => quickAllHere(company)}><CheckCircle weight="fill"/>{busy === company.id ? "Saving…" : "Report all here"}</button> : null}{!headcount.round.closes_at && onSubmit ? <div className="headcount-adjust"><label>Accounted for<input aria-label={`${company.name} accounted count`} type="number" min="0" max={companyExpected} value={draft} onChange={(event) => setDrafts((current) => ({ ...current, [company.id]: event.target.value }))}/></label><button className="secondary" disabled={busy === company.id} onClick={() => submit(company, draft)}>{busy === company.id ? "Saving…" : Number(draft) === companyExpected ? <><CheckCircle weight="fill"/>Save report</> : "Save exception"}</button></div> : null}</div> : null}
                  </div>;
                })}
