                ...
                {shownCompanies.map((company) => {
                  ...
                  return <div className={open ? "headcount-company-card open" : "headcount-company-card"} key={company.id}>
                    ...
                    {open ? <div className="headcount-company-detail">
                      ...
                      {status === "exception" && people.length ? <details className="headcount-reconcile-details"><summary><UsersThree size={17}/> Reconcile missing people <span>{people.filter((person) => personStatusFor(company, person) !== "present").length || ""}</span></summary><div className="headcount-reconcile"><p className="form-hint">Direct head-count status only. Wellness status stays separate. Save the report after choosing a reason.</p>{people.map((person) => <label className="headcount-person-row" key={person.id}><span><b>{person.name}</b><small>{[person.fsyId, person.group].filter(Boolean).join(" · ") || person.registrationId || "Roster person"}</small></span><select disabled={!onSubmit} aria-label={`Head-count status for ${person.name}`} value={personStatusFor(company, person)} onChange={(event) => setPersonDrafts((current) => ({ ...current, [`${company.id}:${person.id}`]: event.target.value }))}>{PERSON_STATUS_OPTIONS.map(([value, text]) => <option key={value} value={value}>{text}</option>)}</select></label>)}</div></details> : null}
