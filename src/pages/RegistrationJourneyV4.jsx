...
export function RegistrationJourney({ view = "desk", sessionId, setImported, capabilities = [], onOperationalDataChanged }) {
  ...
  const filtered = useMemo(() => {
    ...
    return currentRows.filter((row) => {
      ...
      if (!text) return true;
      return matchesRegistrationSearch(row, text, housingByPerson.get(row.participantId));
    }).sort((a, b) => {
  ...
  return <section className={`regjourney regjourney-${view} regjourney-v2 regjourney-v3 regjourney-v4`}>
