...
export function reconcileAccessPeopleV19({ staffRows = [], committeeActive = [], committeePending = [], residualAccounts = [], legacyInvites = [], legacyRequests = [] }) {
  ...
  for (const invite of committeePending) {
    ...
    if (index >= 0) people[index] = { ...people[index], inviteId: people[index].inviteId || invite.inviteId, inviteExpiresAt: people[index].inviteExpiresAt || invite.inviteExpiresAt }; else people.push(invite);
  }
  for (const invite of legacyInvites) {
    const email = normalizeEmail(invite.email); const index = email ? people.findIndex((item) => normalizeEmail(item.accountEmail || item.email) === email) : -1;
    if (index >= 0) people[index] = { ...people[index], legacyInviteId: invite.inviteId, inviteExpiresAt: people[index].inviteExpiresAt || invite.inviteExpiresAt }; else people.push({ ...invite, unmatchedLegacyInvite: true });
  }
...
function RequestReview({ request, onClose, onDecision, onMoveToNew }) {
  ...
  return <DismissibleLayer open onClose={() => !busy && onClose()} title="Review access request" sheet><div className="field-sheet access-v3-review"><button type="button" data-layer-close className="icon-button modal-close" onClick={onClose} disabled={Boolean(busy)} aria-label="Close"><X /></button><span className="kicker">Access request</span><h2>{request.name}</h2><p>{request.email} · {roleLabel(request.role)}</p><div className="notice compact-notice"><WarningCircle /><div><b>Set up this person's current responsibility</b><p>Staff controls responsibility and company scope.</p></div></div>{error ? <MutationFeedback tone="error">{error}</MutationFeedback> : null}<div className="field-sheet-actions"><button className="secondary" disabled={Boolean(busy)} onClick={() => decide("rejected")}>{busy === "rejected" ? "Rejecting…" : "Reject request"}</button><button className="primary" disabled={Boolean(busy)} onClick={onMoveToNew}>Set up access</button></div></div></DismissibleLayer>;
}
...
export function Access({ initialFilter = "", requests = [], invites = [], currentRole = "logistics_admin", currentCapabilities = [], onDecision, onRefreshRoster, onCreateInvite, onRevokeInvite, onCreateRecovery, roster = demoUsers, teams = [], sessionId: requestedSessionId = "", live = false, sessionName }) {
  ...
  const [teamTarget, setTeamTarget] = useState(null); const [reviewRequest, setReviewRequest] = useState(null); const [recoveryResult, setRecoveryResult] = useState(null);
    ...
    const prepare = async () => {
      ...
      try {
        ...
        if (canManage && !preparedSessions.current.has(resolved)) {
          ...
          try { await reconcileNow(resolved); } catch (err) { preparedSessions.current.delete(resolved); throw err; }
        } else await refresh(resolved);
      } catch (err) { if (!cancelled) setError(err.message || "Website access could not be synchronized."); }
  ...
  useEffect(() => { const closeOutside = (event) => document.querySelectorAll(".access-v19 .staff-access-more[open]").forEach((menu) => { if (!menu.contains(event.target)) menu.removeAttribute("open"); }); document.addEventListener("pointerdown", closeOutside); return () => document.removeEventListener("pointerdown", closeOutside); }, []);
  useEffect(() => { document.querySelectorAll(".access-v19 .staff-access-more[open]").forEach((menu) => menu.removeAttribute("open")); }, [view, statusFilter, roleFilter, query, setupTarget, addOpen, teamTarget, confirmDisable, confirmRevoke, confirmRetire]);
  ...
  const residualAccounts = useMemo(() => (roster || []).filter((user) => STAFF_LINKED_ROLES.has(sourceRole(user)) && user.active !== false && sourceUserId(user) && !linkedUsers.has(sourceUserId(user))).map((user) => ({ recordKey: `legacy:${sourceUserId(user)}`, kind: "existing_account", legacyAssignmentId: sourceAssignmentId(user), name: sourceName(user), email: user.email || "", accountEmail: user.email || "", operationalRole: sourceRole(user), companyIds: user.companyIds || user.company_ids || [], userId: sourceUserId(user), accessState: "active" })), [roster, linkedUsers]);
  ...
  const people = useMemo(() => reconcileAccessPeopleV19({ staffRows: directory, committeeActive, committeePending, residualAccounts, legacyInvites, legacyRequests }), [directory, committeeActive, committeePending, residualAccounts, legacyInvites, legacyRequests]);
  const counts = useMemo(() => ({ needs: people.filter((person) => needsAttention(person, now)).length, online: people.filter((person) => person.userId && onlineUserIds.has(person.userId)).length, active: people.filter((person) => person.accessState === "active").length, invited: people.filter((person) => person.accessState === "invited").length, disabled: people.filter((person) => person.accessState === "disabled").length, all: people.length }), [people, onlineUserIds, now]);
  ...
  const repairAccount = async (person) => { const key = personKey(person); setBusyId(key); setError(""); try { const results = await reconcileNow(sessionId); const result = results.find((item) => item.userId === person.userId); if (result && !result.staffId) throw new Error(result.detail || "This account could not be repaired automatically."); setToast({ message: `${person.name}'s existing sign-in is connected` }); } catch (err) { setError(err.message || "This account could not be repaired automatically."); } finally { setBusyId(""); } };
  const toggleAccess = async (person, enabled, { offerUndo = true } = {}) => { if (!person.staffId) return false; const key = personKey(person); if (!live) { setDirectory((current) => current.map((item) => item.staffId === person.staffId ? { ...item, accessState: enabled ? "active" : "disabled", accessEnabled: enabled } : item)); if (offerUndo) setUndo({ person, enabled: !enabled, message: `Sign-in ${enabled ? "enabled" : "disabled"} for ${person.name}` }); return true; } setBusyId(key); setError(""); try { await setStaffWebsiteAccess(person.staffId, enabled); await refresh(); if (offerUndo) setUndo({ person, enabled: !enabled, message: `Sign-in ${enabled ? "enabled" : "disabled"} for ${person.name}` }); return true; } catch (err) { setError(err.message || "Website access could not be changed."); return false; } finally { setBusyId(""); } };
  ...
  return <section className="page access-v4 access-v15 access-v16 access-v17 access-v18 access-v19">
    <PageHead title="Access" sessionName={sessionName} description="Manage who can sign in to FSY Ops. Email identifies the account; Staff responsibility controls what they can see." action={canInviteAnyone ? <button className="primary" onClick={() => setAddOpen(true)}><UserPlus />Add access</button> : null} />
    ...
    <article className="panel access-v17-directory"><header className="access-v17-directory-head"><div><span className="kicker">People</span><h2>{view === "needs" ? "Needs attention" : "Everyone"}</h2><p>{view === "needs" ? "Only people who need a real setup step appear here." : "One row per website identity. Staff responsibility controls website scope."}</p></div><strong>{shown.length}</strong></header>
      {preparing ? <div className="access-v18-directory-loading"><span /><span /><span /></div> : shown.length ? <div className="access-v17-list">{shown.map((person) => {
        ...
        const primary = person.repairNeeded && person.userId ? <button className="primary" disabled={!canManage || rowBusy} onClick={() => repairAccount(person)}>{rowBusy ? "Repairing…" : "Retry repair"}</button> : incomplete ? <button className="primary" disabled={!canManage || rowBusy} onClick={() => setSetupTarget(person)}>Choose companies</button> : person.staffId && person.accessState === "not_enabled" ? <button className="primary" disabled={!canManage || rowBusy} onClick={() => setSetupTarget(person)}>Invite</button> : person.staffId && person.accessState === "disabled" ? <button className="primary" disabled={!canManage || rowBusy} onClick={() => toggleAccess(person, true)}>Enable</button> : (person.unmatchedLegacyInvite || person.legacyRequest) && !person.userId ? <button className="primary" disabled={!canManage || rowBusy || !canAddLeader} onClick={() => startLegacySetup(person)}>Set up access</button> : expired && person.staffId ? <button className="primary" disabled={!canManage || rowBusy} onClick={() => setSetupTarget(person)}>New setup link</button> : null;
        const showMore = Boolean(person.userId || person.inviteId || person.legacyInviteId || person.legacyRequest || person.staffId);
        return <div className={`access-v17-row ${needsAttention(person, now) ? "needs-attention" : ""} ${person.repairNeeded ? "repair-needed" : ""}`} key={key}><div className="access-v17-person"><span className="person-avatar">{initials(person.name)}</span><span><b>{person.name}</b><small>{displayRole(person)}</small>{person.accountEmail || person.email ? <em>{person.accountEmail || person.email}</em> : null}</span></div><div className="access-v17-scope"><small>{scopeHeading(person)}</small><Scope person={person} /></div><div className="access-v17-state"><Status tone={accessTone(person, now)}>{accessLabel(person, now)}</Status><AccountActivity person={person} onlineUserIds={onlineUserIds} activityByUser={activityByUser} now={now} /></div><div className="access-v17-actions">{primary}{showMore ? <details className="staff-access-more"><summary aria-label={`More actions for ${person.name}`}>More</summary><div>{person.staffId && person.operationalRole === "assistant_coordinator" && !person.repairNeeded ? <button disabled={!canManage || rowBusy} onClick={() => setSetupTarget(person)}>Edit companies</button> : null}{person.userId && !person.repairNeeded ? <button disabled={!canManage || rowBusy} onClick={() => setTeamTarget(rosterUser || { userId: person.userId, name: person.name, teamKeys: person.committeeKeys || [] })}>{person.operationalRole === "committee_viewer" ? "Edit committees" : "Committee access"}</button> : null}{onCreateRecovery && person.userId ? <button disabled={!canManage || rowBusy} onClick={() => recovery(person)}><Key />Recovery</button> : null}{person.staffId && person.accessState === "active" && !person.repairNeeded ? <button className="danger-text" disabled={!canManage || rowBusy} onClick={() => setConfirmDisable(person)}>Disable sign-in</button> : null}{(person.inviteId || person.legacyInviteId) && !person.userId ? <button className="danger-text" disabled={!canManage || rowBusy} onClick={() => setConfirmRevoke(person)}>Cancel invite</button> : null}{person.legacyRequest ? <button disabled={!canManage || rowBusy} onClick={() => setReviewRequest(person.legacyRequest)}>Review request</button> : null}{person.repairNeeded && person.userId ? <button className="danger-text" disabled={!canManage || rowBusy} onClick={() => setConfirmRetire(person)}>Remove old access</button> : null}</div></details> : null}</div></div>;
