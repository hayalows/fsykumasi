...
export function Checkin({ participants, cohort, checkedIds = [], onRecord, onAddMissing, live = false, canRecord = true, groupsPublished = false, structureSettings = {}, sessionName }) {
  ...
  const results = useMemo(() => {
    ...
    return withEligibility.filter(({ person }) => `${person.fsyId || ""} ${person.fullName} ${person.preferredName || ""} ${person.registrationId || ""} ${person.unit || ""} ${person.stake || ""}`.toLowerCase().includes(text)).slice(0, 15);
  ...
  return <section className="page">
    <PageHead title="Check-in" sessionName={sessionName} description="Find the person by FSY ID or original registration name, confirm the record, and mark them arrived. Exceptions stay visible without slowing the main line." />
    ...
    <div className="cohort-context checkin-cohort-context"><b>{formatCount(expectedCount)} expected today</b><span>{cohort ? `${formatCount(cohort.records)} registration records · ${formatCount(attentionCount)} need attention` : "Search by FSY ID or original registration details"}</span></div>
