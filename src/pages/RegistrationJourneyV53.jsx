...
import {
  PARTICIPANT_MEMBERSHIP_OPTIONS,
  recordParticipantMembershipCheckin,
  requiresParticipantMembership,
} from "../lib/participant-membership.js";
...
const SOLUTION_META={
  needs_help:{label:"Needs action",help:"Start here"},
  awaiting:{label:"Awaiting approval",help:"Only unresolved"},
  age_review:{label:"Age review",help:"Only unresolved"},
  needs_group:{label:"Needs group",help:"Choose placement"},
  needs_id:{label:"Needs FSY ID",help:"Finish identity"},
  on_site:{label:"On-site",help:"New arrivals"},
};
function countLabel(value,loading=false){return loading?"—":Number(value||0).toLocaleString();}

function mutationErrorMessage(error, fallback = "That step could not be completed.") {
  const raw = String(error?.message || "").trim();
  const technical = /column reference|ambiguous|syntax error|relation .* does not exist|function .* does not exist|pgrst|postgres|sqlstate|schema cache/i.test(raw);
  const safeOperational = /^(Add |Choose |That |This |Enter |Search |A person |First name|Registration management|Only |No available|Company sequence|Recognize )/i.test(raw);
  if (raw && safeOperational && !technical) return raw;
  const friendly = friendlyRuntimeError(error);
  return `${friendly.message || fallback} Reference ${friendly.supportReference}.`;
}
...
function matchesWorkFilter(row,eligibility,filter){
  ...
  if(filter==="awaiting")return Boolean(eligibility?.eligible===false&&(row.registrationStatus==="awaiting"||eligibility?.reason==="Registration is not approved"));
...
export function RegistrationJourneyV53({ view="desk", participants=[], initialGroups=[], initialFilter="", sessionId, capabilities=[], onOperationalDataChanged, onCheckin, onUndoCheckin, onSetOperationalStatus }){
  ...
  const demoMembershipKnown=useRef(new Set());
  ...
  const reload=useCallback(async({initial=false}={})=>{
    ...
    try{
      ...
      setRows(next);
      return next;
    }catch(err){setLoadError(mutationErrorMessage(err,"Registration workspace could not load."));throw err;}
  ...
  const runMutation=async(personId,action,success,{refresh=true,parent=true,onRefreshError}={})=>{
    setBusyId(personId||"workspace");setError("");setMessage(null);
    try{
      const result=await action();
      if(success)setMessage({tone:"success",text:success});
      if(refresh)try{await reload();}catch(err){setError(`Saved, but the latest roster could not refresh. ${mutationErrorMessage(err,"Use Retry to load the latest state.")}`);onRefreshError?.(err);}
      if(parent)refreshParent();
      return result;
    }catch(err){setError(mutationErrorMessage(err));throw err;}
    finally{setBusyId("");}
  };
  ...
  const assignGroup=async(group)=>{if(!selectedRow)return;const targetId=selectedRow.participantId;const onSite=selectedRow.sourceKind==="on_site";const groupName=group.displayName||group.name;let refreshError=null;setPlacementRefreshFailed(false);try{await runMutation(targetId,()=>assignParticipantToGroup(targetId,group.id),onSite?`${selectedRow.fullName} was placed in ${groupName}. Company and FSY ID are ready for check-in.`:`${selectedRow.fullName} was assigned to ${groupName}.`,{onRefreshError:(err)=>{refreshError=err;}});if(refreshError){setPlacementRefreshFailed(true);setMessage(null);setError(`Placement was saved, but the roster could not refresh. ${mutationErrorMessage(refreshError,"Retry to show the new FSY ID before checking in.")}`);return;}const company=placementCompanies.find(item=>item.id===group.companyId);setRows(current=>current.map(item=>item.participantId===targetId?{...item,groupId:group.id,groupName,companyId:group.companyId||item.companyId,companyName:company?.displayName||company?.name||item.companyName}:item));}catch{setPlacementRefreshFailed(false);}};
  ...
  return <section className={`regjourney regjourney-${view} regjourney-v2 regjourney-v3 regjourney-v4 regjourney-v6 regjourney-v29 regjourney-v30 regjourney-v31 regjourney-v53`}>
    {view==="roster"?<><div className="regjourney-roster-head regjourney-roster-head-v2 regjourney-roster-head-v3"><div><span className="kicker">Final roster</span><h2>{finalized?"Roster settled. Finish only the people who still need action.":"Resolve the blocker, then return to normal check-in"}</h2><p>{finalized?"New on-site arrivals move through verification, group placement, FSY ID and check-in without reopening the full roster plan.":"Each participant shows one current blocker and the next useful action."}</p></div><div><b>{countLabel(counts.needs_help,initialLoading)}</b><span>need action</span></div></div>{counts.needs_help?<p className="regjourney-solution-principle-v30"><b>One participant, one next step.</b> Start with Needs action, or search directly.</p>:null}</>:null}
    {error&&!selectedRow&&!onsiteOpen&&!membershipPrompt?<MutationFeedback tone="error">{error}</MutationFeedback>:null}{message?<MutationFeedback tone={message.tone}>{message.text}</MutationFeedback>:null}{loadError&&rows.length?<MutationFeedback tone="error">Roster refresh failed. Showing the last roster that loaded successfully. <button type="button" className="text-action" onClick={()=>reload()}>Retry</button></MutationFeedback>:null}
    <article className="panel regjourney-worklist regjourney-worklist-v2 regjourney-worklist-v3" aria-busy={initialLoading||refreshing}>
      <div className="regjourney-search-row" ref={searchRef}><SearchField value={query} onChange={value=>{setQuery(value);setShown(PAGE_SIZE);}} label={view==="desk"?"Find a participant":"Find someone in Final roster"} placeholder="Name, FSY ID, ward/branch, company, group or room"/>{canManageRegistration&&view!=="desk"?<button type="button" className="secondary regjourney-onsite-button" onClick={openOnsite}><UserPlus/>Add on-site participant</button>:null}</div>
      {view==="desk"&&searching?<div className="regjourney-search-scope-v6"><MagnifyingGlass size={17}/><span><b>Searching all participants</b><small>Ready, checked in and Final roster records are included.</small></span><button type="button" className="text-action" onClick={clearSearch}>Clear</button></div>:null}
      {view==="desk"&&!searching?<><DeskFilters filter={filter} setFilter={setFilter} counts={counts} loading={initialLoading}/>{canManageRegistration?<div className="regjourney-cant-find-v28"><span><b>Someone is not on the list?</b><small>Search first. If no record exists, add them here and finish the four steps.</small></span><button type="button" className="secondary" onClick={openOnsite}><UserPlus/>Add on-site participant</button></div>:null}</>:view==="roster"?<><div className="regjourney-solutions-queues-v28 regjourney-solutions-queues-v30" role="group" aria-label="Final roster queues">{solutionFilters.map(key=>{const meta=SOLUTION_META[key];return <button type="button" key={key} className={filter===key?"active":""} aria-pressed={filter===key} onClick={()=>setFilter(key)}><span>{meta.label}</span><b>{countLabel(counts[key],initialLoading)}</b><small>{meta.help}</small></button>;})}</div><div className="regjourney-roster-filters"><label><span>Other views</span><select value={solutionFilters.includes(filter)?"":filter} onChange={event=>setFilter(event.target.value||"needs_help")}><option value="">Action queues</option><option value="all">Everyone</option><option value="ready">Ready · {countLabel(counts.ready,initialLoading)}</option><option value="arrived">Checked in · {countLabel(counts.arrived,initialLoading)}</option><option value="expected">Yet to arrive · {countLabel(counts.expected,initialLoading)}</option><option value="not_attending">Not attending · {countLabel(counts.not_attending,initialLoading)}</option></select></label><label><span>Source</span><select value={sourceFilter} onChange={event=>{setSourceFilter(event.target.value);setShown(PAGE_SIZE);}}><option value="all">All sources</option><option value="official">Registration list</option><option value="on_site">On-site only</option></select></label></div></>:null}
