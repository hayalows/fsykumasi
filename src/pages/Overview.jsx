...
export function Overview({ live = false, sessionId, setActive, currentRole, currentUser, capabilities = [], fieldSummary = {}, companies = [], imported = [], checkedCount = 0, sessionName }) {
  ...
  const [summary, setSummary] = useState(fallback);const [phase,setPhase]=useState("unknown");const [loading,setLoading]=useState(Boolean(live));const [error,setError]=useState("");
  ...
  const refresh = useCallback(async ({ quiet = false } = {}) => {
    ...
    if (overviewResult.error) { setError("Overview could not refresh. Your workspaces are still available.");if(!quiet)setLoading(false);return; }
    ...
    setPhase(nextPhase);setSummary(overviewResult.data||fallback);setError("");setLoading(false);
  ...
  const effectiveSummary=useMemo(()=>shapeOverviewForPhase(summary,phase),[summary,phase]);
  const baseInbox=buildOperationalInbox({role:currentRole,capabilities,summary:effectiveSummary});
  const inbox=phase==="pre_session"&&baseInbox.whole?{...baseInbox,...preSessionArea(summary)}:baseInbox;
  const name=currentUser?.display_name?.trim().split(/\s+/)[0];const updatedAt=summary?.refreshedAt?new Date(summary.refreshedAt):null;const updatedLabel=updatedAt&&Number.isFinite(updatedAt.getTime())?new Intl.DateTimeFormat(undefined,{hour:"numeric",minute:"2-digit"}).format(updatedAt):"";

  return <section className="page overview-home"><header className="overview-intro"><p className="eyebrow">{sessionName}</p><h1>{name?`Hello, ${name}`:"Overview"}</h1><p>{roleLabel(currentRole)} <span aria-hidden="true">·</span> {inbox.scopeLabel} <span aria-hidden="true">·</span> {phaseLabel(phase)}</p></header>
