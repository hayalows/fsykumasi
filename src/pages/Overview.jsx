  ...
  return <section className="page overview-home">
    ...
    {!inbox?<section className="overview-unavailable" aria-live="polite" aria-busy={loading}><div className="overview-unavailable-mark" aria-hidden="true">{loading?<span/>:<ArrowClockwise/>}</div><div><h2>{loading?"Loading overview":"Overview is not available yet"}</h2><p>{loading?"Getting the latest session information.":"The latest overview could not be loaded. Your other workspaces are still available."}</p>{!loading?<button type="button" className="secondary" onClick={()=>refresh()} disabled={loading}><ArrowClockwise/>Try again</button>:null}</div></section>:<>
      <section className="overview-attention" aria-labelledby="overview-attention-title" aria-busy={loading}>
        <div className="overview-section-head"><h2 id="overview-attention-title">Today</h2>{updatedLabel?<small>Updated {updatedLabel}</small>:null}</div>
        <article className={`overview-focus tone-${inbox.primary.tone||"default"}`}>
          ...
          <div className="overview-focus-copy"><span className="overview-focus-label">{inbox.taskCount?"Next":"Status"}</span><h3>{inbox.primary.title}</h3><p>{inbox.primary.detail}</p></div>
      ...
      {inbox.metrics.length?<section className="overview-area" aria-labelledby="overview-area-title"><div className="overview-section-head overview-scope-head"><div><h2 id="overview-area-title">{inbox.areaTitle}</h2><p>{inbox.areaDetail}</p></div></div><div className="overview-metrics">{inbox.metrics.map(metric=><div key={metric.label} className={metric.attention?"attention":""}><b>{Number(metric.value||0).toLocaleString()}</b><span>{metric.label}</span></div>)}</div></section>:null}
    </>}
