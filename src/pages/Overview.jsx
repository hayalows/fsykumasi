...
export function Overview({ setActive, imported, assignment, pendingAccess, birthdays = [], live = false, companies = [], checkedCount = 0 }) {
  ...
  return <section className="page">
    <PageHead title="Conference operations, without the noise." description="Prepare the session, spot what needs attention, then get leaders back to the youth." action={<button className="primary" onClick={() => setActive(hasParticipants ? "groups" : "registration")}>{primaryLabel}<ArrowRight/></button>}/>
    <div className="brand-banner"><div><span className="kicker">2026 theme</span><h2>Walk With Me</h2><p>Moses 6:34 · Believe, Belong, Become</p></div><img className="theme-identifier" src="/brand/2026-theme-identifier-full-color.png" alt="2026 Walk with Me theme identifier"/></div>
    ...
    <div className="metrics-grid"><Metric label="Youth participants" value={participantCount.toLocaleString()} note={hasParticipants ? "current registration list" : live ? "not imported" : "synthetic rehearsal"}/><Metric label="Checked in" value={checkedCount.toLocaleString()} note={hasParticipants ? `${Math.max(0,participantCount-checkedCount).toLocaleString()} still expected` : "waiting for participant import"} tone="green"/><Metric label="Counselor groups" value={live ? groupCount : (groupCount || "~164")} note={hasGroups ? "published structure" : "not built"} tone="light-blue"/><Metric label="Companies" value={live ? companyCount : (companyCount || "~82")} note={hasCompanies ? "current structure" : "not formed"} tone="yellow"/></div>
    <div className="overview-grid">
      <article className="panel attention"><div className="panel-head"><div><span className="kicker">Attention first</span><h2>What needs action</h2></div><span className="count">{attentionCount}</span></div><button onClick={() => setActive("registration")}><span className="alert-icon"><CloudArrowUp/></span><span><b>{imported.length ? "Registration list is loaded" : "Import registration data"}</b><small>{imported.length ? `${imported.length.toLocaleString()} participant records available · upload a newer export only when needed` : "Use the approved CSV or Excel export"}</small></span><ArrowRight/></button><button onClick={() => setActive("groups")}><span className="alert-icon green"><Buildings/></span><span><b>{hasGroups ? "Review groups, companies & staffing" : "Build groups & companies"}</b><small>{hasGroups ? "Names, staff assignments and grouping rules remain manageable" : "Admins choose the rules, preview, then publish"}</small></span><ArrowRight/></button><button onClick={() => setActive("access")}><span className="alert-icon yellow"><Users/></span><span><b>{pendingAccess} access requests waiting</b><small>Administrative access controls who can manage the session</small></span><ArrowRight/></button></article>
      <article className="panel"><div className="panel-head"><div><span className="kicker">Readiness summary</span><h2>Operational foundation</h2></div><ShieldCheck size={22} className="panel-symbol"/></div><div className="readiness"><div><span>Registration data</span><Status tone={hasParticipants ? "good" : "warn"}>{hasParticipants ? "Imported" : "Not imported"}</Status></div><div><span>Group assignments</span><Status tone={hasGroups ? "good" : "muted"}>{hasGroups ? "Published" : "Waiting"}</Status></div><div><span>Role hierarchy</span><Status>Defined</Status></div><div><span>Access approvals</span><Status tone={pendingAccess ? "warn" : "good"}>{pendingAccess ? `${pendingAccess} pending` : "Clear"}</Status></div></div></article>
