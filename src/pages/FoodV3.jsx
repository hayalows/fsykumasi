...
export function Food({ sessionId, capabilities = [], sessionName, participants = [], live = false }) {
  ...
  const loadServices = useCallback(async ({ quiet = false } = {}) => {
    ...
    try {
      const next = live ? await loadMealServicesV2(sessionId) : demoServices(serviceDate, participants.length);
      setServices(next);
  ...
  const loadRoster = useCallback(async ({ offset = 0, append = false, quiet = false, limit = PAGE_SIZE } = {}) => {
    ...
    try {
      let result;
      if (live) result = await loadMealRosterPageV2({ serviceId: selectedServiceId, query: debouncedMealQuery, companyId: companyFilter, status: mealFilter, limit, offset });
      else {
        const text = debouncedMealQuery.toLowerCase();
        const source = participants.map((person) => ({ personId: person.id, name: person.fullName, fsyId: person.fsyId || "", companyId: person.companyId || "", company: person.company || person.companyName || "Unassigned", group: person.group || person.groupName || "", servedAt: null })).filter((person) => (!companyFilter || person.companyId === companyFilter) && (!text || `${person.name} ${person.fsyId} ${person.company} ${person.group}`.toLowerCase().includes(text)));
  ...
  const dietaryRows = needs.filter((item) => { const text=dietaryQuery.trim().toLowerCase(); if(dietaryFilter==='open'&&item.acknowledged)return false;if(dietaryFilter==='reviewed'&&!item.acknowledged)return false;return !text||`${item.name} ${item.dietaryInformation} ${item.group} ${item.company}`.toLowerCase().includes(text); });
