...
import { Empty, Metric, PageHead, SearchField, Status } from "../components/UI.jsx";
...
function makeStaffSuggestions(groups, companies, staff) {
  ...
  for (const sex of ["Female", "Male"]) {
    ...
    const matchingStaff = shuffled(availableCounselors.filter((person) =>
      !usedCounselors.has(person.id) && (!person.sex || person.sex === sex)
    ));
    const count = Math.min(matchingGroups.length, matchingStaff.length);
    for (let index = 0; index < count; index += 1) {
      const person = matchingStaff[index];
      const group = matchingGroups[index];
      usedCounselors.add(person.id);
      counselors.push({ groupId: group.id, groupName: group.displayName || group.name, staffId: person.id, staffName: person.name });
    }
  }
  ...
  if (assistantCoordinators.length) {
    for (const company of shuffled(companies.filter((item) => !item.assistantCoordinatorIds.length))) {
      const person = [...assistantCoordinators].sort((left, right) =>
        (loads.get(left.id) || 0) - (loads.get(right.id) || 0) || left.name.localeCompare(right.name)
      )[0];
      assistants.push({ companyId: company.id, companyName: company.displayName || company.name, staffId: person.id, staffName: person.name });
      loads.set(person.id, (loads.get(person.id) || 0) + 1);
    }
  }
  return { counselors: shuffled(counselors), assistants: shuffled(assistants) };
}
...
function GroupRow({ group, members, staff, canManage, onRefresh }) {
  ...
  const counselors = staff.filter((person) =>
    person.operationalRole === "counselor"
    && person.registrationStatus === "approved"
    && person.isCurrent !== false
    && (!person.sex || person.sex === group.sex)
  );

  const changeCounselor = async (staffId) => {
    setBusy(true);
    try {
      if (staffId) await assignCounselorToGroup(staffId, group.id);
  ...
  return <div className={open ? "group-operation-block open" : "group-operation-block"}>
    ...
    {open ? <div className="group-disclosed-content">
      <div className="group-members-head"><div><span className="kicker">Youth in this group</span><b>{members.length} assigned</b></div><small>Same-sex counselor group, with ages intentionally blended where the data allows.</small></div>
      ...
      {canManage ? <div className="group-manage-box">
        <label>Counselor<select disabled={busy} value={group.counselorId || ""} onChange={(event) => changeCounselor(event.target.value)}><option value="">No counselor assigned</option>{counselors.map((person) => <option value={person.id} key={person.id}>{person.name}{person.counselorGroupId && person.counselorGroupId !== group.id ? " · already assigned" : ""}</option>)}</select></label>
        {renaming ? <div className="group-editor"><label>Group name<input value={customName} placeholder={group.name} onChange={(event) => setCustomName(event.target.value)}/></label><div className="inline-actions"><button className="secondary compact-button" onClick={() => { setCustomName(group.customName || ""); setRenaming(false); }}>Cancel</button><button className="primary compact-button" disabled={busy} onClick={saveName}><FloppyDisk/>{busy ? "Saving…" : "Save name"}</button></div></div> : <button className="text-action compact-text-action" onClick={() => setRenaming(true)}>Rename counselor group</button>}
      </div> : null}
    </div> : null}
  </div>;
}
...
function CompanyCard({ company, participantsByGroup, staff, canManage, onRefresh }) {
  ...
  const ready = assistants.length > 0 && counselorCount === company.groups.length;
  ...
  return <article className={open ? "company-operation-card progressive-company open" : "company-operation-card progressive-company"}>
    <button className="company-disclosure" onClick={() => setOpen((value) => !value)} aria-expanded={open}>
      ...
      <Status tone={ready ? "good" : "warn"}>{ready ? "Ready" : "Needs staff"}</Status>
    </button>
...
export function Groups({ companyIds = null, participants, assignment, onPublish, live = false, canManage = false, sessionId, onNavigatePeople, sessionName }) {
  ...
  const filteredCompanies = useMemo(() => {
    ...
    return companies.filter((company) => {
      ...
      if (!text) return true;
      const people = company.groups.flatMap((group) => participantsByGroup.get(group.id) || []).map((person) => `${person.fullName} ${person.unit || ""}`).join(" ");
      return `${company.name} ${company.displayName} ${company.scriptureReference || ""} ${company.meetingSpot || ""} ${company.groups.map((group) => group.displayName).join(" ")} ${people}`.toLowerCase().includes(text);
    });
  }, [companies, companyQuery, companyFilter, participantsByGroup]);
