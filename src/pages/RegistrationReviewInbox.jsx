...
export function RegistrationReviewInbox({ imported = [], groups = [], onVerify, onAssign, live = false, canManage = true, canVerify = false, structureSettings = {}, sessionName }) {
  ...
  return <section className="page registration-review-page">
    ...
    {message ? <MutationFeedback>{message}</MutationFeedback> : null}

    <article className="panel quality-review-summary"><div className="panel-head"><div><span className="kicker">Attention first</span><h2>{formatCount(review.totalUnique)} people need review</h2><p>Queues can overlap; this headline counts each person once.</p></div><Status tone={review.totalUnique ? "warn" : "good"}>{review.totalUnique ? "Review inbox" : "Clear"}</Status></div><div className="quality-queue-grid" role="group" aria-label="Review queues">{REVIEW_QUEUE_ORDER.map((key) => { const meta = REVIEW_QUEUE_META[key]; return <button type="button" key={key} aria-pressed={queue === key} className={queue === key ? "quality-queue-card active" : "quality-queue-card"} onClick={() => switchQueue(key)}><strong>{review.counts[key]}</strong><span>{meta.label}</span><small>{meta.short}</small></button>; })}</div><p className="quality-queue-hint">Choose a queue to see its next safe action.</p></article>

    <div className="quality-review-layout">
      <article className="panel quality-review-list"><div className="panel-head"><div><span className="kicker">{REVIEW_QUEUE_META[queue].label}</span><h2>{formatCount(review.counts[queue])} record{review.counts[queue] === 1 ? "" : "s"}</h2><p>{REVIEW_QUEUE_META[queue].help}</p></div></div><SearchField value={search} onChange={(value) => { setSearch(value); setVisible(20); }} label="Search review records" placeholder="Search name, ward, stake or registration ID" /><div className="quality-person-list">{rows.slice(0, visible).map((person) => <button type="button" key={person.id} className={selectedId === person.id ? "quality-person-row selected" : "quality-person-row"} onClick={(event) => openPerson(person, event)}><span className="person-avatar">{initials(person)}</span><span><b>{person.fullName}</b><small>Age {person.age ?? "?"} · {person.unit || "Ward/branch missing"}{person.stake ? ` · ${person.stake}` : ""}</small></span><Status tone={queue === "age_review" || queue === "cancelled" ? "warn" : "muted"}>{queue === "age_review" ? `${person.age ?? "?"} yrs` : REVIEW_QUEUE_META[queue].label}</Status></button>)}{!rows.length ? <div className="empty-inline"><b>No records in this queue</b><span>Nothing currently needs this type of review.</span></div> : null}</div>{rows.length > visible ? <button type="button" className="secondary show-more" onClick={() => setVisible((value) => value + 20)}>Show 20 more · {rows.length - visible} remaining</button> : null}</article>
