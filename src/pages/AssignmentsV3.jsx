        ...
        {filteredStaff.slice(0, visibleStaff).map((person) => {
          ...
          return <div className={`assignments-v3-person-row ${setupNeeded ? "needs-setup" : ""}`} key={person.id}>
            ...
            <div className="assignments-v3-access">
              {committeeStaff ? <><small>Website</small><span className="assignments-v15-committee-note">Committee access is managed in Access</span><button type="button" className="text-action" onClick={() => goToAccess("all")}>Open Access</button></> : accountRole ? accessStatus === "unavailable" ? <><small>Website</small><Status tone="neutral">Status unavailable</Status><button type="button" className="text-action" onClick={() => goToAccess("all")}>Open Access</button></> : <><small>Website</small><Status tone={access?.accessState === "active" ? "good" : access?.accessState === "invited" || access?.accessState === "not_ready" ? "warn" : "neutral"}>{accessStatus === "loading" ? "Checking…" : incomplete ? "Setup incomplete" : accessStateLabel(access?.accessState || "not_enabled")}</Status>{canManage && accessStatus === "ready" && (setupNeeded || access?.accessState === "invited" || access?.accessState === "not_ready") ? <button type="button" className="text-action" onClick={() => access?.accessState === "not_ready" ? setOperationsPerson(person) : setSetupTarget(setupPerson(person))}>{incomplete ? "Finish setup" : access?.accessState === "not_ready" ? "Review staff status" : access?.accessState === "invited" ? "New setup link" : "Set up access"}</button> : null}</> : <small className="assignments-v3-no-access">Website access is not part of this responsibility</small>}
            </div>
          </div>;
        })}
