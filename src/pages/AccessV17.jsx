...
import { LegacyAccessMigrationV17 } from "../components/LegacyAccessMigrationV17.jsx";
...
function sourceEmail(user) { return normalizeEmail(user?.email || user?.accountEmail || user?.account_email || ""); }
function roleCompatible(a, b) { return !a || !b || a === b; }
function demoDirectory() {
  return demoUsers.filter((user) => STAFF_LINKED_ROLES.has(user.roleKey)).map((user, index) => ({
    recordKey: `staff:demo-${index + 1}`,
    kind: "staff",
...
function needsAttention(person, now = Date.now()) {
  if (person.needsConnection || person.legacyRequest || person.unmatchedLegacyInvite) return true;
...
function accessLabel(person, now = Date.now()) {
  if (person.needsConnection) return person.staffId ? "Existing sign-in needs connecting" : "Existing sign-in needs Staff match";
  if (person.legacyRequest) return "Access request needs setup";
  if (person.unmatchedLegacyInvite) return "Older invite needs current setup";
  if (person.operationalRole === "assistant_coordinator" && person.staffId && !person.companyIds?.length) return "Assignment incomplete";
...
function Scope({ person }) {
  ...
  if (person.companyIds?.length) return <span className="access-v3-scope">{person.companyIds.length} company{person.companyIds.length === 1 ? "" : "ies"}</span>;
...
function mergeInvite(person, invite, legacy = false) {
  return {
    ...person,
    name: person.name || invite.name,
    email: person.email || invite.email,
    operationalRole: person.operationalRole || invite.operationalRole,
    companyIds: person.companyIds?.length ? person.companyIds : invite.companyIds || [],
    committeeKeys: person.committeeKeys?.length ? person.committeeKeys : invite.committeeKeys || [],
    committeeNames: person.committeeNames?.length ? person.committeeNames : invite.committeeNames || [],
    accessState: person.userId ? person.accessState : "invited",
    inviteId: person.inviteId || invite.inviteId,
    inviteExpiresAt: person.inviteExpiresAt || invite.inviteExpiresAt,
    legacyInviteId: legacy ? invite.inviteId : person.legacyInviteId,
    unmatchedLegacyInvite: legacy && !person.staffId && !person.userId,
  };
}
function strongStaffMatch(people, account) {
  const accountEmail = normalizeEmail(account.email || account.accountEmail);
  if (accountEmail) {
    const exact = people.filter((item) => item.staffId && normalizeEmail(item.accountEmail || item.email) === accountEmail);
    if (exact.length === 1) return { person: exact[0], confidence: "email" };
  }
  const name = normalizeName(account.name);
  if (!name) return null;
  const exactName = people.filter((item) => item.staffId && normalizeName(item.name) === name && roleCompatible(item.operationalRole, account.operationalRole));
  return exactName.length === 1 ? { person: exactName[0], confidence: "name_role" } : null;
}
export function reconcileAccessPeople({ staffRows = [], committeeActive = [], committeePending = [], legacyAccounts = [], legacyInvites = [], legacyRequests = [] }) {
  let people = [...staffRows, ...committeeActive].map((item) => ({ ...item }));

  for (const account of legacyAccounts) {
    const match = strongStaffMatch(people, account);
    if (match) {
      people = people.map((item) => item === match.person || personKey(item) === personKey(match.person) ? {
        ...item,
        userId: account.userId,
        accountEmail: account.accountEmail || account.email || item.accountEmail,
        legacyAssignmentId: account.legacyAssignmentId,
        legacyRole: account.operationalRole,
        legacyCompanyIds: account.companyIds || [],
        needsConnection: true,
        connectionConfidence: match.confidence,
        accessState: "needs_connection",
      } : item);
    } else {
      people.push({ ...account, needsConnection: true, connectionConfidence: "none", accessState: "needs_connection" });
    }
  }

  for (const invite of committeePending) {
    const email = normalizeEmail(invite.email);
    const index = email ? people.findIndex((item) => normalizeEmail(item.accountEmail || item.email) === email) : -1;
    if (index >= 0) people[index] = mergeInvite(people[index], invite, false);
    else people.push(invite);
  }

  for (const invite of legacyInvites) {
    const email = normalizeEmail(invite.email);
    const index = email ? people.findIndex((item) => normalizeEmail(item.accountEmail || item.email) === email) : -1;
    if (index >= 0) people[index] = mergeInvite(people[index], invite, true);
    else people.push({ ...invite, unmatchedLegacyInvite: true });
  }

  for (const request of legacyRequests) {
    const email = normalizeEmail(request.email);
    const index = email ? people.findIndex((item) => normalizeEmail(item.accountEmail || item.email) === email) : -1;
    if (index >= 0) people[index] = { ...people[index], legacyRequest: request.request, legacyRequestId: request.requestId };
    else people.push({ ...request, legacyRequest: request.request });
  }

  return people.map((person) => ({
    ...person,
    recordKey: person.recordKey || `person:${person.staffId || person.userId || person.inviteId || normalizeEmail(person.email) || normalizeName(person.name)}`,
  }));
}
...
function statusMatches(person, filter, onlineUserIds) {
  ...
  if (filter === "active") return person.accessState === "active" || person.accessState === "needs_connection";
...
function RequestReview({ request, onClose, onDecision, onMoveToNew }) {
  ...
  return <DismissibleLayer open onClose={() => !busy && onClose()} title="Review access request" sheet><div className="field-sheet access-v3-review"><button type="button" data-layer-close className="icon-button modal-close" onClick={onClose} disabled={Boolean(busy)} aria-label="Close"><X /></button><span className="kicker">Access request</span><h2>{request.name}</h2><p>{request.email} · {roleLabel(request.role)}</p><div className="notice compact-notice"><WarningCircle /><div><b>Set this person up from Staff</b><p>The Staff record should remain the source for responsibility, company scope and future access changes.</p></div></div>{error ? <MutationFeedback tone="error">{error}</MutationFeedback> : null}<div className="field-sheet-actions"><button className="secondary" disabled={Boolean(busy)} onClick={() => decide("rejected")}>{busy === "rejected" ? "Rejecting…" : "Reject request"}</button><button className="primary" disabled={Boolean(busy)} onClick={onMoveToNew}>Set up access</button></div></div></DismissibleLayer>;
}
...
export function Access({ initialFilter = "", requests = [], invites = [], currentRole = "logistics_admin", currentCapabilities = [], onDecision, onRefreshRoster, onCreateInvite, onRevokeInvite, onCreateRecovery, roster = demoUsers, teams = [], sessionId: requestedSessionId = "", live = false, sessionName }) {
  ...
  useEffect(() => {
    const closeOutside = (event) => document.querySelectorAll(".access-v17 .staff-access-more[open]").forEach((menu) => { if (!menu.contains(event.target)) menu.removeAttribute("open"); });
    document.addEventListener("pointerdown", closeOutside);
    return () => document.removeEventListener("pointerdown", closeOutside);
  }, []);
  useEffect(() => { document.querySelectorAll(".access-v17 .staff-access-more[open]").forEach((menu) => menu.removeAttribute("open")); }, [view, statusFilter, roleFilter, query, setupTarget, teamTarget, migrationTarget, confirmDisable, confirmRevoke, confirmRetire]);

  const committeeActive = useMemo(() => (roster || []).filter((user) => sourceRole(user) === "committee_viewer" && user.active !== false).map((user) => {
    ...
    return { recordKey: `committee:${sourceUserId(user) || sourceAssignmentId(user) || user.email}`, kind: "committee", name: sourceName(user), email: user.email || "", accountEmail: user.email || "", operationalRole: "committee_viewer", committeeKeys: keys, committeeNames: names, userId: sourceUserId(user), accessState: "active" };
  ...
  const legacyAccounts = useMemo(() => (roster || []).filter((user) => sourceRole(user) !== "committee_viewer" && user.active !== false && sourceUserId(user) && !linkedUsers.has(sourceUserId(user))).map((user) => ({
    recordKey: `legacy:${sourceUserId(user)}`,
    kind: "legacy_account",
    legacyAssignmentId: sourceAssignmentId(user),
    name: sourceName(user),
    email: user.email || "",
    accountEmail: user.email || "",
    operationalRole: sourceRole(user),
    companyIds: user.companyIds || user.company_ids || [],
    committeeKeys: user.teamKeys || user.committeeScope || [],
    userId: sourceUserId(user),
    accessState: "active",
  })), [roster, linkedUsers]);
  ...
  const legacyInvites = useMemo(() => (invites || []).filter((invite) => !invite.staff_id && invite.role !== "committee_viewer" && INVITE_STATES.has(invite.status)).map((invite) => ({
    ...
    kind: "legacy_invite",
    name: invite.display_name || invite.email || "FSY leader",
    email: invite.email || "",
  ...
  const legacyRequests = useMemo(() => (requests || []).filter((request) => request.status === "pending").map((request) => ({
    ...
    request,
    name: request.name || request.email || "FSY leader",
    email: request.email || "",
  ...
  const people = useMemo(() => reconcileAccessPeople({ staffRows: directory, committeeActive, committeePending, legacyAccounts, legacyInvites, legacyRequests }), [directory, committeeActive, committeePending, legacyAccounts, legacyInvites, legacyRequests]);
  const counts = useMemo(() => ({
    needs: people.filter((person) => needsAttention(person, now)).length,
    online: people.filter((person) => person.userId && onlineUserIds.has(person.userId)).length,
    active: people.filter((person) => person.accessState === "active" || person.accessState === "needs_connection").length,
  ...
  const shown = useMemo(() => {
    ...
    return people.filter((person) => {
      if (view === "needs" && !needsAttention(person, now)) return false;
      if (!statusMatches(person, statusFilter, onlineUserIds)) return false;
      if (roleFilter !== "all" && person.operationalRole !== roleFilter) return false;
      if (!text) return true;
      const haystack = `${person.name} ${person.email || ""} ${person.accountEmail || ""} ${displayRole(person)} ${(person.companyNames || []).join(" ")} ${(person.committeeNames || []).join(" ")}`.toLowerCase();
  ...
  const setupKnownAccounts = useMemo(() => {
    if (!setupTarget?.fromLegacy || !setupTarget?.email) return knownAccounts;
    const ownEmail = normalizeEmail(setupTarget.email);
    return knownAccounts.filter((account) => setupAccountEmail(account) !== ownEmail);
  }, [knownAccounts, setupTarget]);
  const setupInvites = useMemo(() => {
    if (!setupTarget?.fromLegacy) return invites;
    return (invites || []).filter((invite) => invite.id !== setupTarget.legacyInviteId && invite.id !== setupTarget.inviteId);
  }, [invites, setupTarget]);
  ...
  const toggleAccess = async (person, enabled, { offerUndo = true } = {}) => {
    ...
    if (!live) {
      ...
      if (offerUndo) setUndo({ person, enabled: !enabled, message: `Sign-in ${enabled ? "enabled" : "disabled"} for ${person.name}` });
      return true;
    }
  ...
  const migrateLegacy = async (person, staffId) => {
    ...
    try {
      ...
      setToast({ message: `${result.name || person.name}'s existing sign-in is now connected to Staff` });
    } catch (err) { setError(err.message || "This existing sign-in could not be connected to Staff."); throw err; }
    finally { setBusyId(""); }
  };
  const retireLegacy = async (person) => {
    const key = personKey(person); setBusyId(key); setError("");
    try { await retireLegacyAccessAccount({ sessionId, userId: person.userId }); await onRefreshRoster?.(); await refresh(); setToast({ message: `Old website access removed for ${person.name}` }); setConfirmRetire(null); }
    catch (err) { setError(err.message || "Old website access could not be removed."); }
    finally { setBusyId(""); }
  };
  ...
  return <section className="page access-v4 access-v15 access-v16 access-v17">
    <PageHead title="Access" sessionName={sessionName} description="Manage who can sign in to FSY Ops and what each person can see." action={canInviteAnyone ? <button className="primary" onClick={startInvite}><UserPlus />Add access</button> : null} />
    ...
    {counts.needs ? <div className="access-v17-attention"><WarningCircle /><div><b>{counts.needs} {counts.needs === 1 ? "person needs" : "people need"} attention</b><p>These are the only people waiting on an account connection, assignment, invite or setup decision.</p></div>{view !== "needs" ? <button className="secondary" type="button" onClick={() => setView("needs")}>Review</button> : null}</div> : <div className="access-v17-ready"><CheckCircle weight="fill" /><div><b>Access is ready</b><p>No account setup work is waiting right now.</p></div></div>}

    <div className="access-v17-toolbar">
      <div className="access-v17-tabs" role="tablist" aria-label="Access views">
        <button type="button" role="tab" aria-selected={view === "needs"} className={view === "needs" ? "active" : ""} onClick={() => setView("needs")}>Needs attention <b>{counts.needs}</b></button>
        <button type="button" role="tab" aria-selected={view === "all"} className={view === "all" ? "active" : ""} onClick={() => setView("all")}>Everyone <b>{counts.all}</b></button>
      </div>
      <div className="access-v17-search"><SearchField value={query} onChange={setQuery} label="Search people" placeholder="Search name, email, role, company or committee" /></div>
      <details className="access-v17-filters">
        ...
        <div className="access-v17-filter-popover">
          ...
          <label><span>Responsibility</span><select value={roleFilter} onChange={(event) => setRoleFilter(event.target.value)}>{ROLE_FILTERS.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label>
          <button type="button" className="text-action" onClick={clearFilters} disabled={statusFilter === "all" && roleFilter === "all"}>Clear filters</button>
        </div>
      </details>
    </div>
    ...
    <article className="panel access-v17-directory">
      <header className="access-v17-directory-head"><div><span className="kicker">People</span><h2>{view === "needs" ? "Needs attention" : "Everyone"}</h2><p>{view === "needs" ? "Only people who need a setup step or decision appear here." : "Current staff, committee members and existing sign-ins appear as one person each."}</p></div><strong>{shown.length}</strong></header>
      {shown.length ? <div className="access-v17-list">{shown.map((person) => {
        const key = personKey(person);
        const rowBusy = busyId === key;
        const incomplete = person.operationalRole === "assistant_coordinator" && person.staffId && !person.companyIds?.length;
        const rosterUser = (roster || []).find((user) => sourceUserId(user) === person.userId);
        const canConnect = person.needsConnection && person.userId;
        const expired = hasExpiredInvite(person, now);
        const primary = canConnect
          ? <button className="primary" disabled={!canManage || rowBusy} onClick={() => setMigrationTarget(person)}>Connect account</button>
          : incomplete
            ...
            : person.staffId && person.accessState === "not_enabled"
              ...
              : person.staffId && person.accessState === "disabled"
                ...
                : (person.unmatchedLegacyInvite || person.legacyRequest) && !person.userId
                  ? <button className="primary" disabled={!canManage || rowBusy || !canAddLeader} onClick={() => startLegacySetup(person)}>Set up access</button>
                  : expired && person.staffId
                    ? <button className="primary" disabled={!canManage || rowBusy} onClick={() => setSetupTarget(person)}>New setup link</button>
                    : null;
        const showMore = Boolean(person.userId || person.inviteId || person.legacyInviteId || person.legacyRequest || person.staffId);
        return <div className={`access-v17-row ${needsAttention(person, now) ? "needs-attention" : ""} ${person.needsConnection ? "needs-connection" : ""}`} key={key}>
          <div className="access-v17-person"><span className="person-avatar">{initials(person.name)}</span><span><b>{person.name}</b><small>{displayRole(person)}</small>{person.needsConnection ? <em>Existing sign-in</em> : null}</span></div>
          <div className="access-v17-scope"><small>{scopeHeading(person)}</small><Scope person={person} /></div>
          <div className="access-v17-state"><Status tone={accessTone(person, now)}>{accessLabel(person, now)}</Status><AccountActivity person={person} onlineUserIds={onlineUserIds} activityByUser={activityByUser} now={now} /></div>
          <div className="access-v17-actions">{primary}{showMore ? <details className="staff-access-more"><summary aria-label={`More actions for ${person.name}`}>More</summary><div>
            {person.needsConnection ? <button disabled={!canManage || rowBusy} onClick={() => setMigrationTarget(person)}>Choose Staff record</button> : null}
            {person.staffId && person.operationalRole === "assistant_coordinator" && !person.needsConnection ? <button disabled={!canManage || rowBusy} onClick={() => setSetupTarget(person)}>Edit companies</button> : null}
            {person.userId && !person.needsConnection ? <button disabled={!canManage || rowBusy} onClick={() => setTeamTarget(rosterUser || { userId: person.userId, name: person.name, teamKeys: person.committeeKeys || [] })}>{person.operationalRole === "committee_viewer" ? "Edit committees" : "Committee access"}</button> : null}
            {onCreateRecovery && person.userId ? <button disabled={!canManage || rowBusy} onClick={() => recovery(person)}><Key />Recovery</button> : null}
            {person.staffId && person.accessState === "active" && !person.needsConnection ? <button className="danger-text" disabled={!canManage || rowBusy} onClick={() => setConfirmDisable(person)}>Disable sign-in</button> : null}
            {(person.inviteId || person.legacyInviteId) && !person.userId ? <button className="danger-text" disabled={!canManage || rowBusy} onClick={() => setConfirmRevoke(person)}>Cancel invite</button> : null}
            {person.legacyRequest ? <button disabled={!canManage || rowBusy} onClick={() => setReviewRequest(person.legacyRequest)}>Review request</button> : null}
            {person.needsConnection && person.userId ? <button className="danger-text" disabled={!canManage || rowBusy} onClick={() => setConfirmRetire(person)}>Remove old access</button> : null}
          </div></details> : null}</div>
        </div>;
      })}</div> : <Empty title={query ? "No person found" : view === "needs" ? "Nothing needs attention" : "No people in this view"} text={query ? "Try another name, email, responsibility, company or committee." : view === "needs" ? "Everyone shown in Access is ready for the state they are in." : "Clear filters or add access for someone new."} action={!query && view === "all" && canInviteAnyone ? <button type="button" className="primary" onClick={startInvite}><UserPlus />Add access</button> : null} />}
    </article>

    {setupTarget ? <LeaderSetupFlow sessionId={sessionId} person={setupTarget.newPerson ? null : setupTarget} allowStaffRoles={canAddLeader} allowCommittee={canInviteCommittee && !setupTarget.fromLegacy} requireEmail teams={teams} knownAccounts={setupKnownAccounts} pendingInvites={setupInvites} onCreateCommitteeInvite={onCreateInvite} onClose={() => setSetupTarget(null)} onComplete={async (person) => {
      if (setupTarget.legacyRequestId && onDecision) await onDecision(setupTarget.legacyRequestId, "rejected", { companyIds: setupTarget.companyIds || [], committeeScope: [] });
      if (setupTarget.legacyInviteId && onRevokeInvite) { try { await onRevokeInvite(setupTarget.legacyInviteId); } catch { /* new invite remains valid even if old cleanup is unavailable */ } }
      if (person.operationalRole !== "committee_viewer") await refresh();
      await onRefreshRoster?.();
      if (setupTarget.fromLegacy) setToast({ message: `${person.name} now uses the current Staff-linked setup` });
    }} /> : null}
    {migrationTarget ? <LegacyAccessMigrationV17 person={migrationTarget} candidates={migrationCandidates} onClose={() => setMigrationTarget(null)} onMigrate={({ staffId }) => migrateLegacy(migrationTarget, staffId)} /> : null}
    ...
    {confirmRetire ? <ConfirmActionSheet open title={`Remove ${confirmRetire.name}'s old website access?`} description="Use this only when the old account should no longer be used." impact="Their old website access and committee memberships for this session will be disabled. A Staff record is not deleted." confirmLabel="Remove old access" cancelLabel="Keep access" busy={busyId === personKey(confirmRetire)} onClose={() => setConfirmRetire(null)} onConfirm={() => retireLegacy(confirmRetire)} /> : null}
