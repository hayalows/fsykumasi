...
export function ArrivalOperations({ sessionId, capabilities = [], onChanged }) {
  ...
  const changeStatus = async (row, nextStatus) => {
    ...
    try {
      let note = "";
      if (nextStatus === "confirmed_not_attending") note = window.prompt("How was this confirmed? e.g. parent contacted, participant contacted, unit leader confirmed") || "";
      await setArrivalStatus(row.participantId, nextStatus, note);
      await reload(); await onChanged?.();
      setMessage({ tone: "success", text: nextStatus === "confirmed_not_attending" ? `${row.fullName} marked confirmed not attending. Their roster place is now available for an approved replacement.` : `${row.fullName} updated.` });
