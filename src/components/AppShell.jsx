...
import { APP_DESCRIPTOR, APP_NAME, APP_SHORT_NAME, APP_THEME, canonicalSessionName } from "../lib/app-meta.js";
import { sessionDayContext } from "../lib/overview-phase.js";
...
import { workspaceDataState } from "../lib/ux-foundation.js";
...
export function AppShell({ active, setActive, attentionCount = 0, currentUser, currentRole = "logistics_admin", currentCapabilities = [], sessionInfo, sessions = [], selectedSessionId = "", onSessionChange, onSignOut, syncError = "", workspacePhase = "ready", lastUpdatedAt = "", onRefresh, children }) {
  ...
  const sessionTitle=canonicalSessionName(sessionInfo?.name||selectedSession?.session_name||demoSession.name);
  const sessionDay=useMemo(()=>sessionDayContext({startsOn:sessionInfo?.starts_on,endsOn:sessionInfo?.ends_on,now:new Date(clock)}),[sessionInfo?.starts_on,sessionInfo?.ends_on,clock]);
  const sessionSubtitle=isTraining?"Safe sandbox · synthetic people only":sessionDay.phase!=="unknown"?sessionDay.compact:"Operations workspace";
  ...
  const dataState=workspaceDataState({online,refreshing:workspacePhase==="loading"||workspacePhase==="refreshing",error:syncError,hasData:workspacePhase!=="loading"&&workspacePhase!=="failed",lastUpdatedAt});
  const connectionLabel=!online?dataState.label:isTraining?"Training data":!isSupabaseConfigured?"Demo data":dataState.state==="ready"?`Connected${freshness?` · ${freshness}`:""}`:dataState.label;
  const connectionShort=!online?"Offline":isTraining?"Training":!isSupabaseConfigured?"Demo":dataState.state==="ready"?"Connected":dataState.label;
  const connectionTone=!online||dataState.state==="failed"?"demo":dataState.state==="stale"?"stale":dataState.state==="updating"||dataState.state==="loading"?"updating":"live";
  ...
  return <div className="app-shell">
    ...
    <aside ref={sidebarRef} className={menu?"sidebar open":"sidebar"} aria-label={`${APP_NAME} navigation`}>
      <div className="brand"><BrandMark compact/><div><b>{APP_NAME}</b><small>{APP_DESCRIPTOR}</small></div><button data-drawer-close className="icon-button sidebar-close" onClick={()=>setMenu(false)} aria-label="Close menu"><X/></button></div>
      <div className={isTraining?"session-badge training":"session-badge"}><span>{isTraining?"Training":sessionInfo?.year||demoSession.year}</span><small>{isTraining?"Synthetic rehearsal workspace":APP_THEME}</small></div>
      ...
      {installPrompt&&!installed?<button type="button" className="sidebar-install" onClick={installApp}><DownloadSimple size={21}/><span><b>Install {APP_SHORT_NAME}</b><small>Open it like an app on this device</small></span></button>:null}
    ...
    <main className="workspace"><header className="topbar"><button ref={menuButtonRef} className="icon-button menu-button" onClick={openMenu} aria-label="Open menu" aria-expanded={menu}><List/></button><div className="session">{sessions.length>1&&onSessionChange?<select className="session-select" value={selectedSessionId} onChange={(e)=>onSessionChange(e.target.value)} aria-label={`Choose ${APP_SHORT_NAME} workspace`}>{sessions.map((item)=><option key={item.session_id} value={item.session_id}>{item.session_status==="training"?`Training · ${canonicalSessionName(item.session_name)}`:canonicalSessionName(item.session_name)}</option>)}</select>:<span>{sessionTitle}</span>}<small>{sessionSubtitle}</small></div><div className="top-actions">{nav.canPeople?<button type="button" className="icon-button global-search-button" onClick={()=>navigate("people")} aria-label="Find a participant or staff member" title="Find someone"><MagnifyingGlass/></button>:null}<span className={`connection ${isTraining?"demo":connectionTone}`} data-backend-environment={supabaseEnvironment} title={connectionLabel} role="status" aria-live="polite"><span className="connection-label-full">{connectionLabel}</span><span className="connection-label-short">{connectionShort}</span></span><button className="icon-button notification-button" onClick={()=>attentionCount&&nav.moreIds.has("access")?navigate("access"):undefined} aria-label={attentionCount?`${attentionCount} access notifications`:"Notifications"}><Bell/>{attentionCount>0?<i>{attentionCount}</i>:null}</button><button className="top-profile-button" onClick={()=>navigate("profile")} aria-label="Open your profile" title="Profile"><AccountAvatar seed={currentUser?.user_id||currentUser?.id} label={`${displayName} profile`} size={34}/></button></div></header>
      ...
      {syncError?<div className="sync-warning" role="alert"><span>{dataState.detail}</span>{onRefresh?<button onClick={onRefresh} disabled={workspacePhase==="refreshing"}>{workspacePhase==="refreshing"?"Updating…":"Try again"}</button>:null}</div>:null}
      {children}
