...
export function StaffOperationsSheet({person,currentRole,onClose,onSaved,assignment}){
 const initial=staffState(person),canConfirm=['session_director','logistics_admin','coordinator','area_advisory_couple'].includes(currentRole),leadership=['logistics_admin','session_director','coordinator','area_advisory_couple'].includes(currentRole);
