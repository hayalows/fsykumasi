...
export function PersonProvider({children,participants=[],assignment,sessionId,onNavigate,view}) {
  ...
  return <PersonContext.Provider value={{open,participantById}}>{children}{identity?<DismissibleLayer open onClose={close} title={identity.name} sheet className="person-peek-layer"><section className="person-peek"><header><div><span className="kicker">{identity.kind==='staff'?'Staff':'Participant'}</span><h2>{identity.name}</h2>{identity.preferredName&&identity.preferredName!==identity.name?<p>Also known as {identity.preferredName}</p>:null}</div><button className="icon-button" data-layer-close onClick={close} aria-label="Close person details">×</button></header><dl>{[
