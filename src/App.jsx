...
const DEMO_CAPABILITIES = ["people_lookup","groups_view","checkin_record","headcount_view","headcount_record","housing_view","housing_manage","housing_export","food_view","food_manage","food_export","registration_view","registration_manage","staff_view","staff_manage","reports_export","access_admin"];
...
export function App() {
  ...
  const [eligibilityMap,setEligibilityMap]=useState(new Map()); const [housingAssignments,setHousingAssignments]=useState([]); const [foodNeeds,setFoodNeeds]=useState([]); const [wellnessEncounters,setWellnessEncounters]=useState([]);
  ...
  const navigate=useCallback((nextView,options={})=>{const view=nextView||"overview";const personId=view==="people"?(options.personId||""):"";setActive(view);setSelectedPersonId(personId);writeWorkspaceLocation(view,{personId,replace:Boolean(options.replace)});},[]);
  useEffect(()=>{const onPopState=()=>{const next=readWorkspaceLocation();setActive(next.view);setSelectedPersonId(next.personId);};window.addEventListener("popstate",onPopState);return()=>window.removeEventListener("popstate",onPopState);},[]);

  const loadFieldData=useCallback(async(sessionId,capabilities=[])=>{
    if(!sessionId)return;
    const [teams,eligibility,staffBdays,housing,food,wellness]=await Promise.all([
      ...
      hasCapability(capabilities,"wellness_private")?loadWellnessEncounters(sessionId):Promise.resolve([]),
    ]);
    setTeamCatalog(teams);setEligibilityMap(eligibility);setStaffBirthdays(staffBdays);setHousingAssignments(housing);setFoodNeeds(food);setWellnessEncounters(wellness);
  },[]);

  const hydrateLive=useCallback(async(sessionOverride,requestedSessionOverride="")=>{
    if(!isSupabaseConfigured)return;setRuntimeError("");const session=sessionOverride===undefined?await getCurrentAuthSession():sessionOverride;setAuthSession(session||null);
    if(!session){setProfile(null);setAccessState([]);setSessionInfo(null);setImported([]);setAccessRequests([]);setLeaderInvites([]);setAccessRoster([]);setTeamCatalog([]);setCompanies([]);setHeadcount({round:null,submissions:[]});setCheckedIds([]);setBirthdays([]);setStaffBirthdays([]);setEligibilityMap(new Map());setHousingAssignments([]);setFoodNeeds([]);setWellnessEncounters([]);setStructureSettings(DEFAULT_STRUCTURE_SETTINGS);setRuntimeStatus("signed-out");return;}
    setRuntimeStatus("loading");
    try{
      const[nextProfile,nextAccessState]=await Promise.all([loadProfile(session.user.id),getMyAccessState()]);setProfile(nextProfile||{user_id:session.user.id,email:session.user.email,display_name:session.user.email});setAccessState(nextAccessState);
      const activeGrants=nextAccessState.filter((item)=>item.active&&item.role);const requestedSession=requestedSessionOverride||selectedSessionId;const granted=activeGrants.find((item)=>item.session_id===requestedSession)||activeGrants.find((item)=>item.session_status!=="training")||activeGrants[0];
      if(!granted){setSessionInfo(null);setImported([]);setAccessRoster([]);setAccessRequests([]);setLeaderInvites([]);setTeamCatalog([]);setCompanies([]);setHeadcount({round:null,submissions:[]});setCheckedIds([]);setBirthdays([]);setStaffBirthdays([]);setEligibilityMap(new Map());setStructureSettings(DEFAULT_STRUCTURE_SETTINGS);setRuntimeStatus("awaiting-access");return;}
      if(granted.session_id!==selectedSessionId)setSelectedSessionId(granted.session_id);const canManageAccess=canApproveAccess(granted.role,granted.capabilities||[]);
      ...
      await loadFieldData(granted.session_id,granted.capabilities||[]);setRuntimeStatus("ready");
    }catch(error){setRuntimeError(error.message||"Unable to load FSY operations data.");setRuntimeStatus("error");}
  },[selectedSessionId,loadFieldData]);

  useEffect(()=>{if(!isSupabaseConfigured)return undefined;let activeSubscription=true;hydrateLive().catch((error)=>{if(activeSubscription){setRuntimeError(error.message||"Unable to connect to Supabase.");setRuntimeStatus("error");}});const unsubscribe=subscribeToAuth((event,session)=>{if(!activeSubscription)return;if(event==="PASSWORD_RECOVERY"){setAuthSession(session);setRuntimeStatus("password-recovery");return;}hydrateLive(session);});return()=>{activeSubscription=false;unsubscribe();};},[hydrateLive]);
  ...
  useEffect(()=>{if(runtimeStatus!=="ready"||!sessionInfo?.id||!["overview","reports"].includes(active))return;const grant=accessState.find((item)=>item.session_id===sessionInfo.id&&item.active&&item.role);loadFieldData(sessionInfo.id,grant?.capabilities||[]).catch(()=>{});},[active,runtimeStatus,sessionInfo?.id]);
  ...
  const handleSignOut=async()=>{setRuntimeError("");try{await signOutAccount();navigate("overview",{replace:true});await hydrateLive(null);}catch(error){setRuntimeError(error.message||"Unable to sign out. Please try again.");throw error;}};
  const handleSessionChange=async(sessionId)=>{const next=accessState.find((item)=>item.session_id===sessionId&&item.active&&item.role);if(!next||sessionId===sessionInfo?.id)return;setSelectedSessionId(sessionId);navigate("overview",{replace:true});if(typeof window!=="undefined"){const url=new URL(window.location.href);if(next.session_status==="training")url.searchParams.set("session",sessionId);else url.searchParams.delete("session");window.history.replaceState({},"",`${url.pathname}${url.search}${url.hash}`);}await hydrateLive(authSession,sessionId);};
  const clearRecoveryUrl=()=>{if(typeof window!=="undefined")window.history.replaceState({},"",window.location.pathname);};

  if(isSupabaseConfigured){if(runtimeStatus==="loading")return <LoadingScreen/>;if(runtimeStatus==="signed-out")return <SignInScreen initialInvite={initialInvite} onSignIn={async(email,password)=>hydrateLive(await signInWithPassword(email,password))} onActivate={async(values)=>hydrateLive(await activateLeaderAccount(values))} onForgot={requestPasswordReset}/>;if(runtimeStatus==="password-recovery")return <PasswordRecoveryScreen onUpdate={updateRecoveredPassword} onCancel={async()=>{clearRecoveryUrl();await hydrateLive(authSession);}}/>;if(runtimeStatus==="error")return <main className="auth-page"><section className="auth-card"><h1>Connection problem</h1><p>{runtimeError}</p><button className="primary full" onClick={()=>hydrateLive(authSession)}>Try again</button></section></main>;if(runtimeStatus==="awaiting-access")return <InviteClaimScreen profile={profile} onClaim={async(code)=>{await claimInviteWhileSignedIn(code);await hydrateLive(authSession);}} onSignOut={handleSignOut}/>;}
  ...
  const rawParticipants=live?imported:imported.length?imported:demoParticipants;const participants=rawParticipants.map((person)=>eligibilityMap.has(person.id)?{...person,serverEligibility:eligibilityMap.get(person.id)}:person);const operationalParticipants=participants.filter((person)=>isOperationalParticipant(person,structureSettings));const cohort=summarizeCohort(participants,structureSettings);const operationalParticipantIds=new Set(operationalParticipants.map((person)=>person.id));const operationalCheckedIds=checkedIds.filter((id)=>operationalParticipantIds.has(id));const pendingAccess=accessRequests.filter((request)=>request.status==="pending").length+leaderInvites.filter((invite)=>invite.status==="pending").length;
  ...
  const handleAddOnSite=live?async(values)=>{await addOnSiteParticipant({sessionId:sessionInfo.id,...values});setImported(await loadParticipants(sessionInfo.id));await reloadEligibility();}:null;
  const handleVerifyOnSite=live?async(participantId,approved)=>{await verifyOnSiteParticipant(participantId,approved);setImported(await loadParticipants(sessionInfo.id));await reloadEligibility();setBirthdays(await loadSessionBirthdays(sessionInfo.id));}:null;
  ...
  const handleAttendance=live?async(participantId,status)=>{await setParticipantAttendanceStatus(participantId,status);await reloadEligibility();}:null;
  ...
  const handlePublishGrouping=live?async(nextAssignment)=>{await publishGroupingPlan(sessionInfo.id,nextAssignment);const[nextParticipants,nextGrouping]=await Promise.all([loadParticipants(sessionInfo.id),loadGroupingPlan(sessionInfo.id)]);setImported(nextParticipants);setCompanies(nextGrouping.companies);setAssignment(nextGrouping);await reloadEligibility();setBirthdays(await loadSessionBirthdays(sessionInfo.id));}:async(nextAssignment)=>{const normalized=normalizeDemoGrouping(nextAssignment);const groupByParticipant=new Map(normalized.groups.flatMap((group)=>group.members.map((person)=>[person.id,group.id])));setImported((current)=>current.map((person)=>groupByParticipant.has(person.id)?{...person,groupId:groupByParticipant.get(person.id)}:person));setCompanies(normalized.companies);setAssignment(normalized);};
  const handleOpenHeadcount=live?async(label)=>{await openHeadcountRound(sessionInfo.id,label);setHeadcount(await loadHeadcount(sessionInfo.id));}:async(label)=>setHeadcount({round:{id:`demo-round-${Date.now()}`,label,opens_at:new Date().toISOString(),closes_at:null},submissions:[]});
  const handleHeadcountSubmit=live?async(values)=>{await submitCompanyHeadcount(values);setHeadcount(await loadHeadcount(sessionInfo.id));}:async(values)=>{const company=companies.find((item)=>item.id===values.companyId);const expectedCount=(company?.groups||[]).reduce((sum,group)=>sum+Number(group.memberCount||group.members?.length||0),0);const submission={company_id:values.companyId,expected_count:expectedCount,accounted_count:Number(values.accountedCount),status:Number(values.accountedCount)===expectedCount?"reported":"exception",note:values.note||""};setHeadcount((current)=>({...current,submissions:[...current.submissions.filter((item)=>item.company_id!==values.companyId),submission]}));};
  ...
  :effectiveActive==="wellness"?<Wellness sessionId={sessionInfo?.id} participants={participants} capabilities={currentCapabilities} sessionName={sessionName}/>
  :effectiveActive==="food"?<Food sessionId={sessionInfo?.id} capabilities={currentCapabilities} sessionName={sessionName}/>
  :effectiveActive==="reports"?<Reports sessionName={sessionName} capabilities={currentCapabilities} companies={companies} headcount={headcount} housingAssignments={housingAssignments} birthdays={birthdays} staffBirthdays={staffBirthdays} foodNeeds={foodNeeds}/>
