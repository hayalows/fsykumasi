...
export function App() {
  ...
  const handleAttendance=async(participantId,status,note="")=>{if(live){await setArrivalStatus(participantId,status,note);await reloadEligibility();await refreshOperationalIdentity();return;}setImported((current)=>{const source=current.length?current:demoParticipants;return source.map((person)=>person.id===participantId?{...person,attendanceStatus:status,attendanceNote:note}:person);});};
  ...
  const handleCheckin=live&&canRecordCheckin?async(participantId,status)=>{await recordCheckin({sessionId:sessionInfo.id,participantId,status});markUpdated();}:null;
