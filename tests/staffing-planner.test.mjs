...
test('source registration status does not decide day-of readiness',()=>{
 const p=person('a',{registrationStatus:'awaiting'});
 assert.equal(canPlanStaff(p),true);
 assert.equal(canServeStaff(p),false);
 assert.equal(staffException(p),'Needs staff confirmation');
 assert.equal(canServeStaff({...p,serviceClearance:'cleared',arrivalState:'arrived'}),true);
 assert.equal(canPlanStaff({...p,registrationStatus:'cancelled'}),true);
 for(const change of [{isCurrent:false},{planningState:'excluded'},{arrivalState:'no_show'},{arrivalState:'left'},{serviceClearance:'not_cleared'}])assert.equal(canPlanStaff({...p,...change}),false);
