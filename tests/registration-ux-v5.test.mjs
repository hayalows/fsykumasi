...
const journey = await readFile(new URL("../src/pages/RegistrationJourney.jsx", import.meta.url), "utf8");
const operations = await readFile(new URL("../src/pages/RegistrationOperationsV2.jsx", import.meta.url), "utf8");
const css = await readFile(new URL("../src/pages/registration-journey.css", import.meta.url), "utf8");
...
test("Check-in desk uses one search and explicit operational states", () => {
  assert.match(journey, /Find participant/);
  assert.match(journey, /Yet to arrive/);
  assert.match(journey, /Checked in/);
  assert.match(journey, /Needs help/);
  assert.match(journey, /On-site/);
  assert.match(journey, /Not attending/);
  assert.match(journey, /Resolve & check in/);
  assert.match(journey, /Complete check-in/);
});
