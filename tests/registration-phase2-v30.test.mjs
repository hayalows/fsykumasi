...
test("Phase 2 gives each normal Registration blocker one next useful action", () => {
  ...
  assert.equal(awaiting.nextAction, "Record the Kumasi decision");
...
test("Solutions rows expose blocker and next action before opening the sheet", async () => {
...
test("Readiness keeps heavy tools lazy and routes participant blockers back to Solutions", async () => {
  const readiness = await read("src/pages/RegistrationReadinessV30.jsx");
...
test("Solutions count is based on actionable blockers rather than the old review inbox", async () => {
  const rows = [
...
test("Phase 2 compact layouts keep the three work areas and Solutions usable at phone widths", async () => {
