...
test("Access uses one invitation entry point for staff leaders and committee members", async () => {
  const [access, flow] = await Promise.all([read("src/pages/AccessV5.jsx"), read("src/components/LeaderSetupFlow.jsx")]);
...
test("committee members and pending committee invites stay visible in the main Access directory", async () => {
  const access = await read("src/pages/AccessV5.jsx");
...
test("modal foundation and Access menus dismiss when the user clicks outside", async () => {
  const [ui, access] = await Promise.all([read("src/components/UI.jsx"), read("src/pages/AccessV5.jsx")]);
...
test("v16 loads after v15 and keeps the v15 interaction language", async () => {
  ...
  assert.ok(v15 > main.indexOf('import "./housing-ux-v14.css";'));
  assert.ok(v16 > v15);
