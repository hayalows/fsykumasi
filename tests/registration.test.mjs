...
test("planning eligibility uses the approved 12 to 19 default and stays configurable", () => {
  ...
  assert.equal(isOperationalParticipant({ ...ready, age: 12 }), true);
  assert.equal(isOperationalParticipant({ ...ready, age: 18 }), true);
  assert.equal(isOperationalParticipant({ ...ready, age: 19 }), true);
